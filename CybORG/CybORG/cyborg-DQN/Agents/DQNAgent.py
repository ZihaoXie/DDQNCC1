...
class DQNAgent(BaseAgent):
    def __init__(self, gamma=0.9, epsilon=0, lr=0.1, n_actions=41, input_dims=(52,),
                 ...
                 replace=1000, algo='DDQN', env_name='Scenario1b', chkpt_dir='chkpt', load=False):  #是否加载已有模型参数
        ...
        self.q_eval = DeepQNetwork(self.lr, self.n_actions, # Q网络
    ...
    def train(self):
        ...
        q_target = rewards + self.gamma*q_next[indices, max_actions] # 计算目标Q值，
        loss = self.q_eval.loss(q_target, q_pred).to(self.q_eval.device) #计算损失
        loss.backward()  # 反向传播
        self.q_eval.optimizer.step()  # 更新当前网络中的参数
