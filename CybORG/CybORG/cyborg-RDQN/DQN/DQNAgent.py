...
class DQNAgent(BaseAgent):
    ...
    def get_action(self, observation, action_space, ignore_epsilon=False): # 是否忽视episode
...
class RNNDQNAgent(DQNAgent):
    def __init__(self, gamma=0.99, epsilon=1, lr=0.0001, n_actions=10, input_dims=(10), lookback_steps=7,
                 ...
                 env=None):
        self.lookback_steps = lookback_steps  # 模型训练时 用来追溯先前历史的步数

        super(RNNDQNAgent, self).__init__(gamma=gamma, epsilon=epsilon, lr=lr, n_actions=n_actions, # 调用DQN的构造函数
        ...
        self.q_eval = DeepRNNNetwork(self.lr, self.n_actions, # 初始化两个RNNDQN 网络
                                        input_dims=self.input_dims,
                                        name=self.env_name+'_'+self.algo+'_q_eval',
        ...
        # 初始化二维数组 obs buffer （lookback_steps input_dim[0]) ,用于agent与环境交互时存储最近7步的的观察值
    ...
    def get_action(self, observation, action_space):
        # 如果输入的obs的形状和 buffer 不一样的话 跟新obs buffer
        if (observation.shape) != self.observation_buffer.shape:
            self.observation_buffer[:-1] = self.observation_buffer[1:]   # 插入agent
            self.observation_buffer[-1] = observation
        else:  # 如果obs 的形状和buffer一样的话， 则将新的观测 赋值给buffer
