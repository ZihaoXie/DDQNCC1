...
class EnvironmentController:
    ...
    提供所有 CybORG 控制器必须实现的抽象方法。 这包括设置和拆卸、修改状态以及从环境中提取数据。
    When both Simulation and Emulation share common functionality, it is implemented here.
    当 Simulation 和 Emulation 共享通用功能时，它会在此处实现。
    ...
    def __init__(self, scenario_path: str, scenario_mod: dict = None, agents: dict = None):
        ...
        self.observation = {}
        self.INFO_DICT['True'] = {}
        # 循环遍历环境中所有的主机，并为每个主机设置一个字典，用于存储关于该主机的各种信息
        ...
        # 循环遍历环境中的所有智能体，并为每个智能体设置一个字典，用于存储该智能体需要获取的初始信息
        ...
        # populate initial observations with OSINT  ---初始观察
        # 使用 OSINT 填充初始观察结果
